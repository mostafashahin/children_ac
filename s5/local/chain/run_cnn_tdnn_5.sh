...
affix=6i   #affix for TDNN+LSTM directory e.g. "1a" or "1b", in case we change the configuration.
common_egs_dir=
reporting_email=
...
local/run_aug_common.sh \
  --suffix $suffix \
  --train-set $train_set \
  --test-sets "$test_sets" \
  --stage $stage --nj $nj \
  --clean-ali tri3_ali_train \
  --aug-list "$aug_list" \
...
if [ $stage -le 12 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" data/train \
    data/lang exp/tri3 exp/tri3_lats_clean
  ...
  steps/copy_lat_dir.sh --nj $nj --cmd "$train_cmd" \
    ...
    data/${train_set}${suffix} exp/tri3_lats_clean $lat_dir || exit 1;
fi

...
aug_list="reverb noise babble clean"
nnet3_affix=   # affix for exp dirs, e.g. it was _cleaned in tedlium.
num_reverb_copies=1
speed_pert=true

# Options which are not passed through to run_ivector_common.sh
affix=7i   #affix for TDNN+LSTM directory e.g. "1a" or "1b", in case we change the configuration.
common_egs_dir=
reporting_email=
...
local/run_aug_common_sp.sh \
  --suffix $suffix \
  --speed-pert ${speed_pert} \
  --train-set ${train_set} \
  --test-sets "$test_sets" \
  --stage $stage --nj $nj \
  --clean-ali tri3_ali_train_sp \
  --aug-list "$aug_list" \
...
lat_dir=exp/chain${nnet3_affix}/${gmm}_${train_set}${suffix}_lats
dir=exp/chain${nnet3_affix}/cnn${affix}${suffix}
train_data_dir=data/${train_set}${suffix}_hires
train_ivector_dir=exp/nnet3${nnet3_affix}/ivectors_${train_set}${suffix}
lores_train_data_dir=data/${train_set}${suffix}

# note: you don't necessarily have to change the treedir name
# each time you do a new experiment-- only if you change the
# configuration in a way that affects the tree.
tree_dir=exp/chain${nnet3_affix}/tree_a${suffix}
...
if [ $stage -le 12 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" data/train_sp \
    data/lang exp/tri3 exp/tri3_lats_clean_sp
  rm exp/tri3_lats_clean/fsts.*.gz # save space
  steps/copy_lat_dir.sh --nj $nj --cmd "$train_cmd" \
    --include-original "$include_original" --prefixes "$prefixes" \
    data/${train_set}${suffix} exp/tri3_lats_clean_sp $lat_dir || exit 1;
fi
...
if [ $stage -le 18 ]; then
  ...
  for data in $test_sets; do
      ...
      steps/nnet3/decode.sh \
          ...
          --online-ivector-dir exp/nnet3${nnet3_affix}/ivectors_${data}${suffix} \
